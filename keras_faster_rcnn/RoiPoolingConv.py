...
class RoiPoolingConv(Layer):
    ...
    def call(self, inputs):
        ...
        def my_pool_rois(x):
            return self._pool_rois(x[0], x[1])

        # 使用「tf.map_fn」生成形状为（batch_size,num_rois，pooled_height，pooled_width，n_channels）的张量。
        roi_layer_output = tf.map_fn(fn=my_pool_rois, elems=inputs, dtype=tf.float32)
        return roi_layer_output

    def _pool_rois(self, feature_map, rois):
        '''
         #获取单张图像所有RoI 的池化结果
        ...
        '''
        def my_pool_roi(x):
            return self._pool_roi(feature_map, x)
